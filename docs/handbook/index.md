...
The handbook explains the library: what each model is, why it is built the way it is, what it assumes, and, where a piece is swappable, the interface a replacement must implement. Nothing on these pages needs to be run. The runnable course is the [Tutorials](../tutorials/index.md) section. Come here when a tutorial leaves you asking *why*.
...
Every page keeps the same shape: what the piece is, why it is built this way, where it breaks, then the interface that drives it. [How it works](../getting-started/how-it-works.md) is the five-minute map of how the pieces connect. To replace one with code of your own, the walkthroughs under [Build your own](../build-your-own/index.md) cover the ground.
