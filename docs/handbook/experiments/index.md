...
result.toDataFrame()  # the same rows, as a pandas DataFrame
...
| the backend | the estimator: `MC(n_encounters=...)` or `IPS(levels=..., n_particles=..., reps=...)` |
...
The backend is the only argument that changes between the two estimators. `MC(n_encounters=...)` counts the losses in
a fixed number of encounters. `IPS(levels=..., n_particles=..., reps=...)` splits the same event into shells and reaches the probabilities
...
[`opencdarr/experiment/`](https://github.com/fazlurnu/OpenCDaRR/tree/main/opencdarr/experiment) package:
`Fixed`, `Sweep`, `Methods`, `MC`, `IPS`, and the `ExperimentResult` with `records()`,
`toDataFrame()`, `cell()` and `plot()`. Each Monte-Carlo cell is one
...
[`estimate_rare_prob`](https://github.com/fazlurnu/OpenCDaRR/blob/main/opencdarr/estimate/ips.py).
...
The notebook for this page is [`examples/handbook/what_you_can_sweep.ipynb`](https://github.com/fazlurnu/OpenCDaRR/blob/main/examples/handbook/what_you_can_sweep.ipynb): the catalogue of every parameter a declaration may carry, and the ones that behave in surprising ways.
