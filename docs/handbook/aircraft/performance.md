...
The [multirotor](kinematics/multirotor.md) reads `v_max`, `ax`, and `yaw_rate_max`. The [fixed-wing](kinematics/fixedwing.md) reads `v_max`, `v_min`, `ax`, `phi_max`, and `roll_rate_max`. A field an airframe does not read stays at its `0.0` default.
...
Construct a `Performance` with the limits you want. For a multirotor the agility parameters are `v_max` and `ax`: how fast it goes, and how sharply it can swing its velocity vector around:
...
That `V²` is unforgiving. At 80 m/s and 30° of bank this aircraft needs a turn more than a kilometre wide, where a small UAV at 17 m/s curls around inside 30 m; see the [fixed-wing turn](kinematics/fixedwing.md) figure. A separation algorithm that ignored the envelope would ask the big aircraft for a manoeuvre it cannot physically make, which is why performance is an input, not a constant baked into the model.
...
The built-in envelopes `M600` (multirotor) and `SMALL_FIXEDWING` (fixed-wing) are two such values, defined in [`performance.py`](https://github.com/fazlurnu/OpenCDaRR/blob/main/opencdarr/performance.py). The quickest start is to copy one and change the numbers, then watch it fly. The notebook for this page is [`examples/handbook/performance.ipynb`](https://github.com/fazlurnu/OpenCDaRR/blob/main/examples/handbook/performance.ipynb): it constructs the two built-ins, triggers the envelope rejection, and computes the turn radii quoted above.
