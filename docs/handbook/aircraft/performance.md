...
The [multirotor](multirotor.md) reads `v_max`, `ax`, and `yaw_rate_max`. The [fixed-wing](fixedwing.md) reads `v_max`, `v_min`, `ax`, `phi_max`, and `roll_rate_max`. A field an airframe does not read stays at its `0.0` default.
...
That `V²` is unforgiving. At 80 m/s and 30° of bank this aircraft needs a turn more than a kilometre wide, where a small UAV at 17 m/s curls around inside 30 m — see the [fixed-wing turn](fixedwing.md) figure. A separation algorithm that ignored the envelope would ask the big aircraft for a manoeuvre it cannot physically make, which is why performance is an input, not a constant baked into the model.
