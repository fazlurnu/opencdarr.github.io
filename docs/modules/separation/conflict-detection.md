...
`StateBased` and `is_los` live in [`opencdarr/cd/`](https://github.com/fazlurnu/OpenCDaRR/tree/main/opencdarr/cd). To write a detector of your own behind the same one-method interface, see [Build your own → Conflict Detection](../../build-your-own/separation-manager/conflict-detection.md).

!!! code "Run it yourself"
    The figure on this page is generated by [`examples/handbook/separation.ipynb`](https://github.com/fazlurnu/OpenCDaRR/blob/main/examples/handbook/separation.ipynb), which also draws the resolution and recovery figures.
