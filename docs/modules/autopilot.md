...
The autopilot decides how the aircraft achieves its mission. Each decision tick it turns that intent into a single [`MotionCommand`](dynamics/index.md#motioncommand), the vehicle-neutral setpoint the airframe then flies. It is the setpoint **producer**; the low-level setpoint **tracker** (the [`Dynamics`](dynamics/index.md) step that banks a fixed-wing or slews a multirotor) lives one layer down. That is the same split [PX4](https://docs.px4.io/main/en/ros/offboard_control) draws between the mission navigator, which holds the plan, and the position controller, which flies it.
