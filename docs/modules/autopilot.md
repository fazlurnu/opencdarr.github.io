...
The autopilot answers one question: **how should the aircraft achieve its mission?** Each decision tick it turns intent into a single [`MotionCommand`](https://github.com/fazlurnu/OpenCDaRR/blob/main/opencdarr/dynamics/base.py), the vehicle-neutral setpoint the airframe then flies. It is the setpoint **producer**; the low-level setpoint **tracker** (the [`Dynamics`](dynamics/index.md) step that banks a fixed-wing or slews a multirotor) lives one layer down. That is the same split [PX4](https://docs.px4.io/main/en/ros/offboard_control) draws between the mission navigator, which holds the plan, and the position controller, which flies it.
...
Three layers act on every tick, in order. The autopilot produces the aircraft's **nominal** command from its own live self-fix. The [separation manager](conflict-detection.md) may **override** that nominal when a conflict is predicted, releasing it back once [recovery](recovery-criteria.md) says the plan is safe again. Whatever command survives is handed to the airframe's `Dynamics.step`, which tracks it under the vehicle's limits. Because the nominal is re-derived each tick from the current state, an aircraft that was pulled off its plan to avoid a conflict simply resumes navigating the moment the override lifts: nothing has to remember where it was going, the mission still says.
...
For a wind-robust path follower this pass leaves L1 as it is: with no wind, a well-tuned L1 is what a fixed-wing needs, and the crab angle that wind introduces is handled in the [Wind](wind.md) module. The multirotor never runs any of this. To it the leg is invisible; it flies to the point and slows to a stop, tracking the position directly.
...
Give no autopilot and the aircraft flies its initial cruise. The figures on this page are drawn by [`scripts/handbook/autopilot.py`](https://github.com/fazlurnu/OpenCDaRR/blob/main/scripts/handbook/autopilot.py):

## Want a different guidance law?

A loiter that spirals in, a Dubins path planner, a follow-the-leader rule, anything. You can [Build your own → Autopilot](../build-your-own/autopilot.md).
