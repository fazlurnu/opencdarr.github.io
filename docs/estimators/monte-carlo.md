...
[`examples/handbook/monte_carlo.ipynb`](https://github.com/fazlurnu/OpenCDaRR/blob/main/examples/handbook/monte_carlo.ipynb) makes the figure on this page. There is no separate figure script for this page.

!!! code "Run it yourself"
    The same notebook uses each setting on this page. It shows one call, then the unit of the estimate, then the record of each encounter, then the reproducibility and the chunks, and last the limit of the method. Run the notebook from the start to the end to reproduce the numbers.
