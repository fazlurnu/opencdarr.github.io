...
At 30° both resolvers sit at zero losses in 500 encounters, which is where plain Monte Carlo runs out of resolution. Swapping `backend=MC(...)` for `backend=IPS(...)` estimates the same quantity by splitting, with the rest of the declaration unchanged — see [rare-event simulation](../rare-event/index.md).
