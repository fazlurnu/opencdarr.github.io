...
A [scenario](../scenario/index.md) is one run. An **experiment** is many runs, and you declare it
...
distribution: that role is one level below, in the [scenario](../scenario/index.md).
...
- **[Ex: pairwise conflict](example-pairwise-conflict.md)** — MVP against VO on two aircraft, as
  the crossing becomes shallow and the position fix becomes worse.
- **[Ex: random traffic](example-random-traffic.md)** — the same declaration on a fleet in a disc,
