...
**Modularity.** Every component listed in **[Modules](modules/index.md)** is an abstract base class with a single method to implement. Changing an experiment means changing an implementation. Comparing your conflict resolution algorithm is changing one input argument away.
...
| [`RecoveryCriterion`](modules/separation/recovery-criteria.md) | Decide when it is safe to disengage the separation algorithm |
